...
public class MuteRequestTest {
  private static final User USER_JIMMY =
      User.builder()
          ...
          .setIdentity("User")
          .build();
  private static final MuteResponse MUTE_JIMMY_RESPONSE = MuteResponse.create(USER_JIMMY);
  ...
  @Test
  public void muteCreateShouldReturnBlockResponse() throws Exception {
    final String expectedUrl =
        "https://api.stocktwits.com/api/2/mutes/create/176389.json?access_token=testToken";
    stubResponse(MUTE_JIMMY_RESPONSE);

    MuteResponse response =
    ...
    assertThat(response).isEqualTo(MUTE_JIMMY_RESPONSE);
    assertThat(actualRequest.uri().toString()).isEqualTo(expectedUrl);
    assertThat(actualRequest.method()).isEqualTo("POST");
  }

  @Test
  public void muteDestroyShouldReturnBlockResponse() throws Exception {
    final String expectedUrl =
        "https://api.stocktwits.com/api/2/mutes/destroy/176389.json?access_token=testToken";
    stubResponse(MUTE_JIMMY_RESPONSE);
  ...
  private void stubResponse(MuteResponse response) throws Exception {
